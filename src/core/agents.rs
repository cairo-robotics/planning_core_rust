use nalgebra::{Point3, Quaternion, UnitQuaternion, Vector3, Translation3, Isometry3};
use pyo3::prelude::*;
...
use crate::optimization::opt::{OptimizationEngineNLopt, OptimizationEngineOpen};
...
impl Agent {
    #[new]
    fn new(fp: String, position_mode_relative: bool, rotation_mode_relative: bool) -> Self {
        let agent_vars = init_agent_vars(fp.clone(), position_mode_relative, rotation_mode_relative);
    ...
    fn forward_kinematics(&mut self, j_config: Vec<f64>) -> PyResult<Vec<Vec<f64>>> {
        ...
        let position_end_chain = ee_poses[1].0;
        let quat_chain = ee_poses[1].1.clone().into_inner();
    ...
    fn relaxed_inverse_kinematics(&mut self, pos_arr: Vec<f64>, pos_length: i32, 
        quat_arr: Vec<f64>, quat_length: i32) -> PyResult<Opt> {
        ...
        for i in 0..self.relaxed_ik.lock().unwrap().vars.robot.num_chains {
        ...
        let ja = self.relaxed_ik.lock().unwrap().solve(&g);    
        let len = ja.len();
...
fn init_agent_vars(fp: String, position_mode_relative: bool, rotation_mode_relative: bool) -> AgentVars {
    let ifp = InfoFileParser::from_yaml_path(fp.clone());
        ...
        let collision_nn_path = get_path_to_src()
            + "/config/collision_nn_rust/"
